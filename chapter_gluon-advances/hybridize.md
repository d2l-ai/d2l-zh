...
正如大家所预期，在运行`e = add(a, b)`时，Python会做加法运算并将结果存储在变量`e`，从而令程序的状态发生了改变。类似地，后面的两个语句`f = add(c, d)`和`g = add(e, f)`会依次做加法运算并存储变量。

虽然命令式编程写起来很自然，但它的主要缺点是可能会慢。一方面，我们不断跟可能较慢的Python运行环境交互：即使我们重复调用了`add`函数三次，我们还是会跟Python运行环境交互三次。另一方面，我们需要保存变量`e`和`f`的值直到`fancy_func`中所有语句执行结束。这是因为在执行`e = add(a, b)`和`f = add(c, d)`之前我们并不知道变量`e`和`f`会不会被程序的其他部分使用。
