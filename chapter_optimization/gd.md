...
尽管*梯度下降*（gradient descent）很少被直接用于深度学习，
但了解它是理解下一节随机梯度下降算法的关键。
例如，由于学习率过大，最优化问题中可能会出现分歧，这种现象早已在梯度下降中出现。
...
为了简单起见，我们选用目标函数$f(x)=x^2$来展示如何实现梯度下降。
...
使用梯度下降法迭代$x$ 10次，我们可以看到，$x$的值最终将接近最优解。
...
学习率$\eta$可由算法设计者设置。
如果我们使用的学习率太小，将导致$x$的更新非常缓慢，需要更多的迭代。
...
也就是说，:eqref:`gd-taylor-2`中的$\mathcal{O}(\eta^2 f'^2(x))$可能变得显著了。在
这种情况下，$x$的迭代不能保证降低$f(x)$的值。
...
为了演示非凸函数的情况，考虑函数$f(x) = x \cdot \cos(cx)$，其中$c$为某些常数。
这个函数有无穷多个局部最小值。
根据我们选择的学习率，我们最终可能只会得到许多解的一个。
下面的例子说明了高学习率会如何导致局部最小值的解的“不切实际”。
...
为了避免繁琐的符号，我们将$\mathbf{H} \stackrel{\mathrm{def}}{=} \nabla^2 f(\mathbf{x})$定义为$f$的Hessian，这是$d \times d$矩阵。
当$d$的值很小且问题很简单时，$\mathbf{H}$很容易计算。
但是对于深度神经网络而言，考虑到$\mathcal{O}(d^2)$个条目的存储成本，$\mathbf{H}$可能非常大。
此外，通过反向传播进行计算可能过于昂贵。
让我们姑且先忽略这些考量，看看会得到什么算法。

毕竟，$f$的最小值满足$\nabla f = 0$。
遵循 :numref:`subsec_calculus-grad` 中的微积分规则，
...
也就是说，作为优化问题的一部分，我们需要反转Hessian $\mathbf{H}$。
...
* 学习率的大小很重要：学习率太大会是模型发散，学习率太小会没有进展。
* 梯度下降会在求局部极小值中陷住，而得不到全局最小值。
* 在高维中，调整学习率是很复杂的。
* 预处理有助于调节比例。
...
1. 设计一个定义在$\mathbb{R}^2$上的目标函数，它的梯度下降非常缓慢。提示：对不同的坐标使用不同的比例。
1. 使用预处理实现牛顿方法的轻量级版本：
    1. 使用对角Hessian作为预条件。
