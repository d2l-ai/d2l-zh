...
动量法的提出是为了应对梯度下降的上述问题。由于小批量随机梯度下降比梯度下降更为广义，以下将沿用[“小批量随机梯度下降”](minibatch-sgd.md)一节中时间步$t$的小批量梯度$\boldsymbol{g}_t$的定义。设时间步$t$的自变量为$\boldsymbol{x}_t$、学习率为$\eta_t$。
在时间步$0$，动量法创建速度变量$\boldsymbol{v}_0\in\mathbb{R}^d$，并将其元素初始化成0。在时间步$t>0$，动量法对每次迭代的步骤做如下修改：
