...
(不需要下面这句，因为词并不是默认用onehot表示）
这通常需要把维数为词典大小的高维空间嵌入到一个更低维数的连续向量空间。
把词映射为实数域上向量的技术也叫词嵌入（word embedding）。近年来，词嵌入已逐渐成为自然语言处理的基础知识。那么，我们应该如何使用向量表示词呢？
...
## word2vec
（需要一个过渡段：word embedding将词映射到一个空间保持相似度。然后再是word2vec。建议不要提Google，因为前面工作的都没有提公司，就引用paper就是了。）
...
（感觉这一章公式太多，理解起来比较麻烦。最好多解释下每个公式。而且建议把建议把模型，词的向量表示，概率的计算放一起。梯度计算拿出来放后面。最后说近似计算。近似计算的motivation不是特别清楚，需要再突出下。另外，最好提前说下梯度计算和近似都不需要懂，如果skip不影响后面阅读。）
