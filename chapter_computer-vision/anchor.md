...
现在我们可以标注锚框的类别和偏移了。如果一个锚框没有被分配真实边界框，我们只需将该锚框的类别设为背景；否则，设锚框类别为分配的真实边界框的类别，并将偏移标注为：

（真实边界框中心x坐标 - 锚框中心x坐标）／ 猫框宽，（真实边界框中心y坐标 - 锚框中心y坐标）／ 猫框高，（真实边界框宽 - 锚框宽）／ 猫框宽，（真实边界框高 - 锚框高）／ 猫框高

我们通常将类别为背景的锚框称为负类锚框，其余则称为正类锚框。
...
返回的第一项是真实边界框相对锚框的标注偏移量，负类锚框的偏移标注为0。
