...
在训练集中，我们将每个锚框视为一个训练样本。为了训练目标检测模型，我们需要为每个锚框标注两类标签：一是锚框所含目标的类别；二是锚框与真实边界框的距离。在目标检测时，我们首先生成多个锚框，然后为每个锚框预测所含目标类别以及与真实边界框的距离，接着根据预测的距离调整锚框位置，最后筛选需要输出的锚框。


我们知道，在目标检测的训练集中，每个图像已标注了真实边界框的位置以及所含目标的类别。在生成锚框之后，我们主要依据与锚框相似的真实边界框的位置和类别信息为锚框标注。那么，该如何搜索与锚框相似的真实边界框呢？
...
依此类推，直到矩阵$\boldsymbol{X}$中所有$n_b$列元素全部被丢弃。这个时候，我们已为$n_b$个锚框各分配了一个真实边界框。接下来，我们将遍历剩余的$n_a - n_b$个锚框：给定锚框$A_i$，找到与它交并比最大的真实边界框$B_j$，如果该交并比大于预先设定的阈值（例如0.5），则为锚框$A_i$分配真实边界框$B_j$，否则锚框$A_i$无分配的真实边界框。
...
现在我们可以标注锚框了。如果一个锚框无分配的真实边界框，我们只需将该锚框所含目标的类别设为背景；否则，设锚框与分配的真实边界框所含目标的类别相同，并为该锚框标注分配的真实边界框的位置减去锚框位置所得的差值，即锚框与真实边界框的距离。我们通常将含有背景的锚框称为负类锚框，其余则称为正类锚框。由于$x$轴和$y$轴的坐标分别已除以图像的宽和高，锚框与真实边界框的距离在-1和1之间。
