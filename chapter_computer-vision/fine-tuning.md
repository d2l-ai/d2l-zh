...
在前面的章节中，我们讨论了如何在 Fashion-MNIST 训练数据集上训练模型，只有 60000 张图像。
我们还描述了 ImageNet，这是学术界中使用最广泛的大型图像数据集，它拥有 1000 多万张图像和 1000 个对象实物。
但是，我们通常遇到的数据集的大小介于两个数据集中的大小之间。 
...
在本节中，我们将介绍迁移学习中的常见技巧 : *微调*（fine-tuning）. As shown in :numref:`fig_finetune`，微调包括以下四个步骤： 
...
在ResNet的全局平均池后，完全连接的层汇集转换为 ImageNet 数据集的 1000 个类输出。
