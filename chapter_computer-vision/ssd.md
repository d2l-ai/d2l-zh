...
![单发多框检测模型主要由一个基本网络块和若干多尺度特征块串联而成。](../img/ssd.svg)
:label:`fig_ssd`
...
基本网络块用于从输入图像中抽取特征。
为了计算简洁，我们构造了一个小的基本网络，该网络串联3个高和宽减半块，并逐步将通道数翻倍。
...
* 单发多框检测是一种多尺度目标检测模型。基于基本网络块和各个多尺度特征块，单发多框检测生成不同数量和不同大小的锚框，并通过预测这些锚框的类别和偏移量检测不同大小的目标。
