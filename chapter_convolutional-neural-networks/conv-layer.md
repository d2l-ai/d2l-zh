...
上节我们解析了卷积层的理论原理，现在我们看看它的实际应用。由于卷积神经网络是人类探索图像数据的结晶，本节我们将以图像为例。
...
当卷积窗口滑动到新一个位置时，包含在该窗口中的输入子传感器与卷积核张量进行元素相乘，得到的张量相加为一个单一的标量值，由此我们得出了这一位置的输出张量值。
在如上例子中，输出张量的四个元素由二维互相关运算导出，这个输出高度为 $2$ 、宽度为  $2$ ，如下所示：
...
一般而言，输出大小略小于输入大小。这是因为卷积核的宽度和高度大于1，
而卷积核只与图像中每个大小完全适合的位置进行互相关运算。
所以，输出大小等于输入大小 $n_h \times n_w$ 减去卷积核大小 $k_h \times k_w$ 再加  $1 \times 1$ ，即：
...
卷积层对输入和内核权重进行互相关运算，并在添加标量偏差之后产生输出。
所以，训练卷积层的两个参数是内核权重和标量偏差。
在训练卷积层模型时，我们通常随机初始化内核权重，就像我们之前随机初始化完全连接层一样。
...
我们将如上卷积核 `kernel_size` 为 $h \times w$ 的卷积层称为 $h \times w$ 卷积层，其的高度和宽度分别为 $h$ 和 $w$ 。
...
接下来，我们构造一个高度为 $1$ 、宽度为 $2$ 的内核 `K` 。当进行互相关运算时，如果水平相邻的两元素相同，则输出为零，否则输出为非零。
...
现在我们将二维图像输入转置，进行如上的互相关运算。
其输出如下，之前检测到的垂直边缘消失了。
不出所料，这个内核 `K` 只可以检测垂直边缘，而非水平边缘。
...
如果我们只需寻找黑白边缘，那么以上 `[1, -1]` 的边缘检测器足以。然而，当有了更复杂数值的内核，或者连续的卷积层时，我们不可能手动设计过滤器。那么我们是否可以学习以  `X` 生成 `Y` 的卷积内核呢？
...
在 $10$ 次迭代之后，误差已经骤降。现在我们来看看我们所学的卷积核的权重张量。
...
在CNN中，对于某一层的任何元素 $x$ ，其 *感受野* （Receptive Field）是指在前向传播期间可能影响 $x$ 计算的所有元素（来自所有先前层）。
