...
回顾之前我们将 softmax 回归模型（:numref:`sec_softmax_scratch`）和全连接层模型（:numref:`sec_mlp_scratch`）应用于 Fashion-MNIST 数据集中的服装图片。
...
由于完整的数据集位于主内存中，因此在模型使用 GPU 计算数据集之前，我们需要将其复制到显存中。
:end_tab:
