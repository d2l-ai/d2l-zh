...
在几种情况下，我们采用了影响输出大小的填充和条带卷积等技术。作为动机，请注意，由于内核的宽度和高度通常大于 $1$，在应用多个连续卷积后，我们倾向于结束输出比输入小得多。如果我们从 $240 \times 240$ 像素的图像开始，$10$ 卷积图层将图像减少到 $200 \times 200$ 像素，将图像切下 $30\ %$ 的图像，并将其抹掉原始图像边界上的任何有趣的信息。
*填充 * 是处理此问题的最流行的工具。
...
一般来说，如果我们总共添加 $p_h$ 行填充（大约一半在顶部，一半在底部）和总计 $p_w$ 列填充（左侧大约一半，右侧半），则输出形状将为

$$(n_h-k_h+p_h+1)\times(n_w-k_w+p_w+1).$$

这意味着输出的高度和宽度将分别增加 $p_h$ 和 $p_w$。

在许多情况下，我们需要设置 $p_h=k_h-1$ 和 $p_w=k_w-1$，使输入和输出具有相同的高度和宽度。这样可以在构建网络时更容易地预测每个图层的输出形状。假设 $k_h$ 在这里是奇数的，我们将在高度的两侧填充 $p_h/2$ 行。如果 $k_h$ 是偶数，则一种可能性是在输入顶部填充 $\lceil p_h/2\rceil$ 行，在底部填充 $\lfloor p_h/2\rfloor$ 行。我们将以相同的方式填充宽度的两侧。

CNN 通常使用具有奇数高度和宽度值的卷积内核，例如 1、3、5 或 7。选择奇数内核大小的好处是，我们可以保持空间维度，同时在顶部和底部使用相同数量的行填充，左侧和右侧的列数相同。

此外，这种使用奇核和填充来精确保存维度的做法也具有文书效益。对于任何二维张量 `X`，当内核的大小是奇数，并且所有侧面的填充行数和列数相同，生成与输入高度和宽度相同的输出时，我们知道输出 `Y[i, j]` 是通过输入和卷积核的互相关计算的。窗口以 `X[i, j]` 为中心。
...
为了简洁起见，当输入高度和宽度两侧的填充数量分别为 $p_h$ 和 $p_w$ 时，我们称之为填充 $(p_h, p_w)$。具体来说，当 $p_h = p_w = p$ 时，填充是 $p$。当高度和宽度上的步幅分别为 $s_h$ 和 $s_w$ 时，我们称之为步幅 $(s_h, s_w)$。具体而言，当我们的步幅为 $s_h = s_w = s$ 时，步幅为 $s$。默认情况下，填充为 0，步幅为 1。在实践中，我们很少使用不均匀的步幅或填充，也就是说，我们通常有 $p_h = p_w$ 和 $s_h = s_w$。
