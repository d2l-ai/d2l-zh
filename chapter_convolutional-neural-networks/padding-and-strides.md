...
在前面的示例 :numref:`fig_correlation` 中，输入的高度和宽度都为$3$，卷积核的高度和宽度都为 $2$ ，生成的输出表示的维数为 $2\times2$。
...
CNN 卷积核的高度和宽度通常为奇数，例如 1、3、5 或 7。
...
比如，在下面的示例中，我们创建一个所有边为 3 的二维卷积层，并在所有侧面填充 1 个像素。给定所有边为 $8$ 的输入，则输出的高度和宽度也是 8。
