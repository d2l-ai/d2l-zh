...
在 :numref:`sec_seq2seq` 中研究机器翻译问题时，我们设计了一个基于两个循环神经网络的“编码器－解码器”架构，用于序列到序列的学习。具体来说，循环神经网络编码器将可变长度序列转换为固定形状的上下文变量（context variable），然后循环神经网络解码器每次根据上一步生成的标记和上下文变量，一个一个地生成输出（目标）序列的标记。虽然每步解码中都使用了 *相同的* 用于描述整个输入序列的上下文变量，但是并非所有的输入（源）标记都对解码某个特定标记有用。
