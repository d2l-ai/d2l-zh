...
与 :numref:`sec_seq2seq_training` 类似，我们在这里指定超参数，并且实例化包含了 Bahdanau 注意力的编码器和解码器，然后对这个模型进行基于机器翻译案例的训练。由于新增了注意力机制，这个训练会比没有注意力机制的 :numref:`sec_seq2seq_training` 慢得多。
...
* 当预测一个标记时，如果输入的标记不是全部都相关的，那么包含了 Bahdanau 注意力的循环神经网络“编码器－解码器”会有选择地聚合输入序列的不同部分，这种选择性聚合是通过将上下文变量视为可加性注意力池化的输出来实现的。
