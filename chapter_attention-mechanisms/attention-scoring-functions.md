...
在 :numref:`sec_nadaraya-waston` 中，我们使用高斯核来建模查询和键之间的关系。可以将 :eqref:`eq_nadaraya-waston-gaussian` 中的高斯核的指数部分视为 *注意力评分函数*（简称 *评分函数*），在这个函数结果的基础上再进行 softmax 操作。通过上述步骤，我们获得了与键配对的值的概率分布（注意力权重）。最后，注意力池化的输出就是基于这些注意力权重的值的加权和。
...
用数学语言描述，假设我们有一个查询 $\mathbf{q} \in \mathbb{R}^q$ 和 $m$ 键值对 $(\mathbf{k}_1, \mathbf{v}_1), \ldots, (\mathbf{k}_m, \mathbf{v}_m)$，其中 $\mathbf{k}_i \in \mathbb{R}^k$ ，$\mathbf{v}_i \in \mathbb{R}^v$。注意力池化 $f$ 就被表示成值的加权和：

$$f(\mathbf{q}, (\mathbf{k}_1, \mathbf{v}_1), \ldots, (\mathbf{k}_m, \mathbf{v}_m)) = \sum_{i=1}^m \alpha(\mathbf{q}, \mathbf{k}_i) \mathbf{v}_i \in \mathbb{R}^v,$$
:eqlabel:`eq_attn-pooling`

其中查询 $\mathbf{q}$ 和键 $\mathbf{k}_i$ 的注意力权重（标量）是通过注意力评分函数 $a$ 再经过 softmax 运算得出的，该函数将两个向量映射成一个标量：

$$\alpha(\mathbf{q}, \mathbf{k}_i) = \mathrm{softmax}(a(\mathbf{q}, \mathbf{k}_i)) = \frac{\exp(a(\mathbf{q}, \mathbf{k}_i))}{\sum_{j=1}^m \exp(a(\mathbf{q}, \mathbf{k}_j))} \in \mathbb{R}.$$
:eqlabel:`eq_attn-scoring-alpha`

正如我们所看到的，不同的注意力评分函数 $a$ 决定了不同的注意力池化操作。在本节中，我们将介绍两个流行的评分函数，稍后将用他们来实现更复杂的注意力机制。
...
正如我们刚才提到的，softmax 运算用于输出一个概率分布来作为注意力权重。在某些情况下，并非所有的值都应该被纳入注意力池化中。例如，为了在 :numref:`sec_machine_translation` 中高效地进行小批量处理，某些文本序列被填充了没有意义的特殊词符号。为了将注意力集中在仅作为值的有意义的词符号上，我们可以指定一个有效的序列长度（以词符号个数表示），以便在计算 softmax 时过滤掉超出此指定范围的那些位置。通过这种方式，我们可以在下面的 `masked_softmax` 函数中实现这样的 *masked softmax 操作*，其中任何超出有效长度的结果都被遮盖并置为零。
