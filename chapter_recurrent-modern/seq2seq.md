...
值得一提的是，我们在《8.5. 循环神经网络的从零开始实现》中使用的独热编码（One-hot ）和此处的嵌入层功能相近，都是为了获得输入序列中每个词元的特征向量。
而不同的地方在于，使用One-hot方法编码的向量会很高维也很稀疏，假设我们拿到了一个包含2000个词的字典，每一个词会被一个包含2000个整数的向量来表示，其中1999个数字是0，如果字典再大一点，模型容易受到“维度灾难”（Curse of Dimensionality）的影响，即不仅计算效率会大打折扣，而且需要更多的数据才能使得模型收敛。
因此，使用嵌入层可以使得输入序列中每个词元的特征向量的维数降低，缓解“维度灾难”。此外，嵌入层中也具有可训练的参数，使得我们输入序列中每个词元的特征向量可以学习到它们词与词之间的一些相似性，这也是独热编码所不具备的特性。
