...
训练深度神经网络是困难的。让他们在合理的时间内收敛可能是棘手的。在本节中，我们将介绍 * 批量归一化 *，这是一种流行且有效的技术，可持续加速深层网络 :cite:`Ioffe.Szegedy.2015` 的收敛速度。再加上后面在 :numref:`sec_resnet` 中介绍的残余块，批量归一化使得从业人员能够例行地培训拥有 100 多个层的网络。

## 培训深层网络

为了激励批量归一化，让我们回顾训练机器学习模型和神经网络时出现的一些实际挑战。
