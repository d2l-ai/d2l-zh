...
可见，$L_2$范数正则化令权重$w_1$和$w_2$先乘以$1-\lambda$然后再减去没有惩罚项的梯度。在深度学习里，$\lambda$通常是小于1的正的常数，所以它也被称为权重衰减（weight decay）。权重衰减相当于对模型参数增加了额外的限制，这可能对过拟合有效。实际场景中，我们有时也在惩罚项中添加偏差元素的平方和。
