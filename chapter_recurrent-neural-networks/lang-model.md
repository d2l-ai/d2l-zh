...
语言模型（language model）是自然语言处理的重要技术。（说下为什么将lang model：我们将用这个应用来介绍rnn。）

自然语言处理中最常见的数据是文本数据。实际上，我们可以把一段自然语言文本看作是一段离散的时间序列。假设一段长度为$T$的文本中的词依次为$w_1, w_2, \ldots, w_T$，那么在离散的时间序列中，$w_t$（$1 \leq t \leq T$）可看作是在时间步（time step）$t$的输出或标签。给定一个长度为$T$的词的序列：$w_1, w_2, \ldots, w_T$，语言模型将计算该序列的概率：
...
（解释实际计算太复杂，所以有引入n-gram的理由）

## $N$元语法

（要么先说如何只依赖于前n-1个词，然后说这是马尔科夫假设。要么先解释下什么是马尔科夫）
...
（本章->后续小节，读者可能不一定注意了章和节的区别，所以感觉这句话像本节讨论）
...
（可以改成，语言模型计算一个句子出现的概率，是自然语言处理的重要技术。n-gram基于$n-1$阶马尔可夫假设简化计算。）
