...
换言之，每个中间活性值$h$以*丢弃概率*$p$由随机变量$h'$替换，如下所示：
...
我们将输入`X`通过暂退法操作，丢弃概率分别为0、0.5和1。
...
并且可以为每一层分别设置丢弃概率：
常见的技巧是在靠近输入层的地方设置较低的丢弃概率。
下面的模型将第一个和第二个隐藏层的丢弃概率分别设置为0.2和0.5，
并且暂退法只在训练期间有效。
...
将丢弃概率作为唯一的参数传递给它的构造函数。
在训练时，`Dropout`层将根据指定的丢弃概率随机丢弃上一层的输出（相当于下一层的输入）。
在测试时，`Dropout`层仅传递数据。
