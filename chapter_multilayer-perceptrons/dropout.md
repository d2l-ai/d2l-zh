...
在整个训练过程的每一次迭代中，暂退法包括在计算下一层之前将当前层中的一些节点置零。
...
通常，我们在测试时禁用暂退法。给定一个训练好的模型和一个新的样本，我们不会丢弃任何节点，因此不需要标准化。然而，也有一些例外：一些研究人员使用测试时的暂退法作为估计神经网络预测的“不确定性”的启发式方法：如果预测在许多不同的暂退法掩码上都是一致的，那么我们可以说网络更有自信心。
