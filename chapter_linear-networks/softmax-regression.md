...
您可能会想是否可以将未归一化的预测 $o$ 直接视作我们感兴趣的输出。但是，将线性层的输出直接视为概率时存在一些问题。一方面，没有限制这些数字的总和为1。另一方面，根据输入的不同，它们可以为负值。这些违反了:numref:`sec_prob` 中提出的概率基本公理。
...
为了将未归一化的预测变换为非负并且总和为1，同时要求模型保持可导。我们首先对每个未归一化的预测求幂，这样可以确保输出非负数。然后再除以它们的总和，这样可以确保它们的总和为1：
...
相对于一次处理一个样本，小批量样本的矢量化加快了 $\mathbf{X}和\mathbf{W}$ 的矩阵-向量乘法运算。由于$\mathbf{X}$ 中的每一行代表一个数据点，且softmax操作本身可以*按行*（rowwise）执行。所以，对于$\mathbf{O}$的每一行，我们首先对所有项进行幂运算，然后通过求和对它们进行标准化。
