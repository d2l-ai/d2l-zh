...
深度学习框架通过自动计算导数（即 *自动求导*）来加快这项工作。实际上，根据我们设计的模型，系统构建一个 *计算图*（computational graph），来跟踪哪些数据通过哪些操作组合而产生输出。自动求导使系统能够随后反向传播梯度。
...
作为一个演示样本，假设我们有兴趣对函数 $y = 2\mathbf{x}^{\top}\mathbf{x}$ 相对于列向量 $\mathbf{x}$求导。首先，我们创建变量 `x` 并为其分配一个初始值。
...
甚至在我们计算$y$相对于$\mathbf{x}$的梯度之前，我们需要一个地方来存储它。
重要的是，我们不会在每次对一个参数求导时分配新的内存，因为我们经常会数千次或数百万次地更新相同的参数，可能很快就会耗尽内存。注意，标量值函数相对于向量$\mathbf{x}$的梯度本身是向量值的，并且与$\mathbf{x}$具有相同的形状。

```{.python .input}

# 我们通过调用attach_grad`来为一个张量的梯度分配内存
x.attach_grad()
# 在我们计算一个关于`x`的梯度后，我们将能够通过' grad '属性访问它，它的值被初始化为0
x.grad
```
...
由于 `x` 是一个长度为 4 的向量，因此执行 `x` 和 `x` 的内积，得到了我们赋值给 `y` 的标量输出。接下来，我们可以通过调用反向传播函数来自动计算`y`相对于 `x` 每个分量的梯度，并打印这些梯度。
...
函数 $y = 2\mathbf{x}^{\top}\mathbf{x}$ 相对于$\mathbf{x}$ 的梯度应为$4\mathbf{x}$。让我们快速验证我们想要的梯度是否正确计算。
...
从技术上讲，当 `y` 不是标量时，向量`y`关于向量`x`的导数的最自然的解释是一个矩阵。对于高阶和高维的 `y` 和 `x`，求导的结果可以是一个高阶张量。
...
使用自动求导的一个好处是，即使构建函数的计算图需要通过 Python控制流（例如，条件、循环或任意函数调用），我们仍然可以计算结果变量的梯度。在下面的代码中，注意 `while` 循环的迭代次数和 `if` 语句的结果都取决于输入 `a` 的值。
