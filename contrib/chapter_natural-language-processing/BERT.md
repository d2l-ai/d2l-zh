...
为了训练深度双向的表示，BERT设计了一种名为遮蔽语言模型的任务，类似于完形填空的猜词任务。具体来说，就是随机将一定比例的输入标记替换为遮蔽标记“[MASK]”，然后预测这些被遮蔽的标记。即将遮蔽标记对应的输入隐藏向量输入一个单层网络，用softmax计算词汇表中每个单词的概率，以预测遮蔽标记应该对应哪个词。
...
mlm_pred = mlm_decoder(encodings, mlm_positions) # (批量大小, 遮蔽数目, 此表大小)
