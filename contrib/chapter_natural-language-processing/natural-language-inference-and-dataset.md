...
在“文本情感分类：使用循环神经网络”一节中，我们介绍了使用循环神经网络进行文本分类的模型。在文本分类任务中，是给定单个文本序列，然后识别该文本序列的类别。而在现实场景中，我们有时需要给定两个句子，针对这两个句子的关系进行分类，即句对分类任务。在这里所说的两个句子之间的关系可以是语义相似关系或者推理关系。例如，购物网站的在线客服系统，需要判断用户发起的提问和知识库中已有问题是否具有同样的含义。在这种情况下，针对单个文本序列进行分类的模型无法胜任。
