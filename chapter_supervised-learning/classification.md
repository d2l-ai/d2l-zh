...
不妨从另一个角度来理解交叉熵损失函数。在训练模型时，对于训练数据集上每个样本，总体上我们希望模型对这些样本的标签类别的预测概率尽可能大，也就是希望模型尽可能容易输出真实的标签类别。设$p_{\text{label}_i}$是模型对样本$i$的标签类别的预测概率。同样设训练数据集的样本数为$n$。最大化训练数据集上标签类别的联合预测概率$\prod_{i=1}^n p_{\text{label}_i}$等价于最小化交叉熵损失函数。这是因为对数函数单调递增（最大化$\prod_{i=1}^n p_{\text{label}_i}$ 等价于最大化$\sum_{i=1}^n \log p_{\text{label}_i}$ ），且最大化对数函数等价于最小化该函数的相反数（最大化$\sum_{i=1}^n \log p_{\text{label}_i}$等价于最小化$-\sum_{i=1}^n \log p_{\text{label}_i}$）。
