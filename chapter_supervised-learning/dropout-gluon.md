...
在多层感知机中Gluon实现的基础上，我们只需要在全连接层后添加Dropout层并指定丢弃概率。Dropout层检查当前是否在训练模式，若是在训练模式，它将以指定的丢弃概率来丢弃元素。
...
这部分依然和多层感知机中的训练和测试没有多少区别。因为Dropout层会自行检测当前是否在训练模式，在测试模型时，我们也并不需要修改模型的定义。
