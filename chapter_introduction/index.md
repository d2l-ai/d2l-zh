...
直到最近，我们每天与之交互的每个计算机程序几乎都是由软件开发人员按照**第一性原则**（first principle）编码的。假设我们要编写一个应用程序来管理电子商务平台。在白板（whiteboard）上冥思几个小时之后，我们可能提出一个可行的解决方案，它的大致思路看起来像这样：

1. 用户通过在 Web 浏览器或移动应用程序中运行的界面与应用程序进行交互；
2. 我们的应用程序与商业级数据库引擎进行交互，以跟踪每个用户的状态并维护历史交易记录；
3. 在我们应用程序的核心，我们的应用程序的业务逻辑（您可能会说，是“大脑”（brain））详细地阐明了程序在每种可能的情况下应采取的适当措施。

为了建立我们应用程序的 “大脑”，我们必须逐步应对每一个可能遇到的极端情况，并制定适当的规则。每当买家单击将商品添加到购物车时，我们都会向购物车数据库表中添加一个条目，将该用户的 ID 与请求产品的 ID 关联起来。虽然很少有开发人员第一次完全正确（可能需要一些测试才能解决问题），但在大多数情况下，我们可以从第一性原则编写这样的程序，并在看到真正的客户之前自信地启动它。我们能够根据第一性原则设计自动化系统，通常在新的情况下驱动正常运行的产品和系统，这是一个非凡的认知壮举。而且，当您能够设计出可以 $100\%$ 地运行的解决方案时，就不应该使用机器学习。

幸运的是，对于越来越多的机器学习 (ML) 科学家社区来说，我们想要自动化的许多人类的聪明才智并不容易。想象一下，用你所知道的最聪明的头脑围绕白板，但这次你正在解决以下问题之一：

* 编写一个程序，依据给定的地理信息, 卫星图像和过去天气的尾随窗口（trailing window）预测明天的天气。
* 编写一个程序，提出一个以自由格式文本表示的问题，并正确回答该问题。
* 编写一个程序，给出一个图像可以识别它包含的所有人，并绘制其轮廓。
* 编写一个程序，向用户提供他们很可能喜欢但在自由浏览过程中不可能遇到的产品。

在这些情况下，即使是精英程序员也无法从头开始编写解决方案。这种情况的原因可能会有所不同。有时候，我们正在寻找的程序遵循一种随着时间的推移而变化的模式，我们需要我们的程序来适应。在其他情况下，关系（例如像素和抽象类别之间的关系）可能太复杂，需要数千或数百万的计算，这些计算超出了我们的意识（即使我们的眼睛毫不费力地管理任务）。ML 是一项强大的技术研究，可以**从经验中学习**。随着 ML 算法积累了更多的经验，通常采用观测数据或与环境交互的形式，其性能会提高。将这与我们的确定性电子商务平台进行对比，该平台根据相同的业务逻辑执行，无论积累多少经验，直到开发人员自己**学习**并决定是时候更新软件。在这本书中，我们将教你机器学习的基础知识，并特别关注深度学习，这是一套强大的技术，推动计算机视觉、自然语言处理、医疗保健和基因组学等不同领域的创新。

## 一个激励性的例子

在我们开始写作之前，这本书的作者们，像许多苦力一样，不得不成为咖啡因。我们跳上车，开始驾驶。使用 iPhone， Alex 大喊 “嘿 Siri”，唤醒手机的语音识别系统。然后 Mu 发出命令 “Blue Bottle 咖啡店的方向”。手机很快显示了他的命令的转录。它还认识到，我们正在寻求方向，并启动了地图应用程序来满足我们的要求。启动后，地图应用程序会识别多条路线。在每条路线旁边，手机显示预测的运输时间。虽然我们为了教学方便而制作这个故事，但它表明，在短短几秒钟的时间内，我们与智能手机的日常互动可以集成多种机器学习模型。

想象一下，只需编写一个程序来响应一个 *唤醒词*，如 “Alexa”，“Okay, Google” 或 “Siri”。如 :numref:`fig_wake_word` 所示，只需要一台计算机和一个代码编辑器就可以自己在一个房间里进行编码。你将如何利用第一性原则编写这样的程序？想想一下... 这个问题很困难。每秒钟，麦克风将收集大约 $44\,000$ 个样本。每个样本都是声波幅度的测量值。什么样的规则可以从原始音频片段可靠地映射到有关片段是否包含唤醒字的自信预测 `{yes, no}`？如果你卡住了，不要担心。我们也不知道如何从头开始编写这样的程序。这就是为什么我们要使用 ML。

![识别唤醒词](../img/wake-word.svg)
:label:`fig_wake_word`

这是一个诀窍。通常情况下，即使我们不知道如何明确地告诉计算机如何从输入到输出的映射，我们仍然能够自己完成认知的壮举。换句话说，即使你不知道如何编程，让计算机识别单词 “Alexa”，但你自己能够识别 “Alexa” 这个词。有了这种能力，我们可以收集一个大量的 **数据集**，其中包含音频样本和标签（是否包含包含唤醒词）。在 ML 方法中，我们不尝试设计一个系统明确地识别唤醒词。相反，我们定义了一个灵活的程序，其行为由一些 **参数** 确定。然后我们使用数据集来确定最好的参数集, 那些提高我们程序的性能相对于某种程序的性能对感兴趣的任务.

你可以把参数视为旋钮，我们可以转动它，操纵程序的行为。固定参数，我们称该程序为 **模型**。我们可以通过操作参数生成的所有不同程序（输入-输出映射）的集合称为 * 系列 * 模型。使用我们的数据集选择参数的 *meta 程序 * 称为 * 学习算法 *。
